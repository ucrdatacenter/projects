...
# Data link (Download as a CSV (Comma Separated Values) file):
...
tibble |> 
  mutate(age_in_20_years = age + 20) |> 
  filter(age_in_20_years > 40) |> 
  print()
...
data |> 
  filter(Fabric == "MYCENEAN") |> 
  count()

data |>
  filter(Fabric == "CYPRIOT, BRONZE AGE") |> 
  count()
